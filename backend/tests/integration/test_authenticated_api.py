...
ROUTE_MATRIX: dict[tuple[str, str], dict[Role, int]] = {
    ("GET", "/api/v1/me"): {Role.ANALYST: 200, Role.MANAGER: 200, Role.ADMIN: 200},
...
def test_route_protection_matrix(
    ...
    response = client.request(
        method, path, headers=bearer(token_signer, principal_for(role))
    )
    assert response.status_code == expected, (
        f"{role.value} {method} {path}: expected {expected}, got {response.status_code}"
    )
...
def test_me_reports_server_authoritative_permissions(
    client: TestClient, token_signer: LocalIdentityProvider, analyst: Principal
) -> None:
    body = client.get("/api/v1/me", headers=bearer(token_signer, analyst)).json()
...
def test_admin_cannot_decide_approvals(
    client: TestClient, token_signer: LocalIdentityProvider, admin: Principal
) -> None:
    """Segregation of duties, verified through the API rather than only in a unit test."""
    body = client.get("/api/v1/me", headers=bearer(token_signer, admin)).json()
    assert "approval:decide" not in body["permissions"]
    assert "user:manage" in body["permissions"]


def test_token_for_an_unprovisioned_organization_is_refused(
    client: TestClient, token_signer: LocalIdentityProvider
) -> None:
...
def test_dev_login_issues_a_usable_token(client: TestClient) -> None:
    ...
    login = client.post("/api/v1/auth/dev-login", json={"email": "analyst@acme.test"})
...
def test_dev_login_for_an_unknown_user_is_refused(client: TestClient) -> None:
    response = client.post("/api/v1/auth/dev-login", json={"email": "nobody@acme.test"})
    assert response.status_code == 401
...
def test_login_reconciles_role_from_the_token(
    ...
    body = client.get("/api/v1/me", headers=bearer(token_signer, promoted)).json()
